      ...
      <v-expansion-panel>
        <v-expansion-panel-content>
          <div
            ...
            <v-subheader
              class="pa-0 ma-0"
              style="height: auto">Log</v-subheader>
          </div>
          <div
            v-for="(l, k) in logs"
            :key="k"
            class="px-3">
            <div
              v-if="k && format(l.time) !== format(logs[k - 1].time)"
              style="clear: both; height: 2px"
              class="grey darken-1">
              <v-divider/>
            </div>
            <v-divider
            ...
            <div
              style="float: left"
              class="pr-3 grey--text">{{ format(l.time) }}</div>
            <div
              :class="{[`${COLORS[l.level]}--text`]: true}"
              style="float: right"
              class="body-2">{{ l.msg }}</div>
          </div>
          <div
            :class="{'mb-3': logs.length}"
            style="clear: both"/>
        </v-expansion-panel-content>
      </v-expansion-panel>
    </div>
